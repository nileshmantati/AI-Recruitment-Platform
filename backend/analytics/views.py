...
class RecruiterAnalyticsView(APIView):
    ...
    def get(self, request):
        ...
        total_applications = Application.objects.filter(
            job__recruiter=request.user).count()
        shortlisted = Application.objects.filter(
            job__recruiter=request.user, status='SHORTLISTED').count()

        # 3. Get Chart Data (Applications per Job)
        # Using Django's annotate to count applications for each job efficiently
        jobs_with_counts = Job.objects.filter(
            recruiter=request.user).annotate(app_count=Count('applications'))

        # Formatting data specifically for Recharts
        chart_data = [
            {
                # Truncate long names
                "name": job.title[:15] + "..." if len(job.title) > 15 else job.title,
