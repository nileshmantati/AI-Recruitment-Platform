...
class HiringWorkflow(models.Model):
    company = models.OneToOneField(
        Company, on_delete=models.CASCADE, related_name='hiring_workflow')
...
class HiringStage(models.Model):
    workflow = models.ForeignKey(
        HiringWorkflow, on_delete=models.CASCADE, related_name='stages')
...
class TeamMember(models.Model):
    ...
    user = models.ForeignKey(settings.AUTH_USER_MODEL,
                             on_delete=models.CASCADE, related_name='team_roles')
    role = models.CharField(
        max_length=20, choices=ROLE_CHOICES, default='RECRUITER')
...
class AppearanceSettings(models.Model):
    user = models.OneToOneField(
        settings.AUTH_USER_MODEL, on_delete=models.CASCADE, related_name='appearance_settings')
    theme = models.CharField(max_length=20, choices=[(
        'light', 'Light Mode'), ('dark', 'Dark Mode'), ('system', 'System Theme')], default='light')
    accent_color = models.CharField(max_length=50, default='blue')
    sidebar_style = models.CharField(max_length=20, choices=[(
        'expanded', 'Expanded'), ('compact', 'Compact')], default='expanded')
...
class IntegrationSettings(models.Model):
    company = models.ForeignKey(
        Company, on_delete=models.CASCADE, related_name='integrations')
    # e.g., 'Google Calendar', 'Zoom', 'Slack'
