...
class Company(models.Model):
    ...
    cover_banner = models.ImageField(upload_to='company/banners/', blank=True, null=True)
    ...
    working_mode = models.CharField(max_length=10, choices=WORKING_MODE_CHOICES, blank=True, null=True)
    working_days = models.CharField(max_length=100, blank=True, null=True) # e.g., "Monday - Friday"
    working_hours = models.CharField(max_length=100, blank=True, null=True) # e.g., "9:00 AM - 5:00 PM"
    benefits = models.JSONField(default=list, blank=True)
    perks = models.JSONField(default=list, blank=True)
    languages = models.CharField(max_length=255, blank=True, null=True) # e.g., "English, Spanish"
...
class CompanySocial(models.Model):
    company = models.OneToOneField(Company, on_delete=models.CASCADE, related_name='socials')
...
class CompanyBranding(models.Model):
    ...
    primary_color = models.CharField(max_length=7, default='#0f172a') # Hex color
...
class CompanyAIPreferences(models.Model):
    company = models.OneToOneField(Company, on_delete=models.CASCADE, related_name='ai_preferences')
